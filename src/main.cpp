    ...
    while (!WindowShouldClose()) {
        ...
        if (currentState == STATE_PLAYING) {
            ...
            if (showInteractMenu) {
                ...
                float currentAccel = 3500.0f; float currentMaxSpeed = 700.0f;
                ...
                if (IsKeyPressed(KEY_E)) {
                    ...
                    if (grabbedEntityIndex != -1) {
                        ...
                        std::sort(interactTargets.begin(), interactTargets.end(), [&](int a, int b) { return Vector2Distance(player.position, entities[a].position) < Vector2Distance(player.position, entities[b].position); });
            ...
            if (executeAction) {
                if (!isDropMenu) {
                    ...
                    if (res != CHEM_NONE) {
                        if (res == CHEM_ATTACHED) { item.attachedTo = actionTargetIdx; item.isGrabbed = false; item.isUsing = false; item.velocity = {0,0}; item.zVelocity = 0; grabbedEntityIndex = -1; }
                    } else {
                        item.attachedTo = actionTargetIdx; item.isGrabbed = false; item.isUsing = false; item.velocity = {0,0}; item.zVelocity = 0; grabbedEntityIndex = -1;
                    }
                }
            }
            ...
            RenderWorld(renderTarget, camera, dt, entities, player, grabbedEntityIndex, hazVis);
