...
int main(void) {
    ...
    for (const auto& sp : spawnList) {
        ...
        if (globalHitboxes.find(baseName) == globalHitboxes.end()) {
            if (baseName == "Handbook" || baseName.find("Brochure") != std::string::npos) {
                globalHitboxes[baseName] = { {{-15, 0, -20}, {15, 5, 20}} }; 
            } 
            else if (baseName == "PandoraBox") {
                globalHitboxes[baseName] = { {{-30, 0, -30}, {30, 40, 30}} }; 
            } 
            else {
                globalHitboxes[baseName] = { {{-20, 0, -20}, {20, 40, 20}} }; 
            }
            globalTweaks[baseName] = { 100.0f, {0,0,0}, 0.0f }; 
        }
    }
    ...
    // --- FEATURE: Global Rotation Mode Toggle ---
    ...
    while (!WindowShouldClose()) {
        ...
        // --- FEATURE: Toggle Rotation Mode ---
        if (IsKeyPressed(KEY_LEFT_ALT)) {
            isRotationMode = !isRotationMode;
            if (isRotationMode) SaveState(entities); 
        }
        ...
        if (currentMode == MODE_LEVEL) {
            ...
            // --- FIX: Only zoom camera if mouse is inside the 3D space, preventing UI zoom bugs ---
            ...
            if (mousePos.x > UI_WIDTH && pendingSpawnIdx == -1) { 
                ...
                // --- FEATURE: Advanced Rotation Mode Handling ---
                if (isRotationMode) {
                    if (!selectedIndices.empty()) {
                        float rotDelta = GetMouseDelta().x * 0.5f;
                        if (rotDelta != 0.0f) {
                            for (int idx : selectedIndices) {
                                entities[idx].stateValue += rotDelta;
                                if (entities[idx].stateValue >= 360.0f) entities[idx].stateValue -= 360.0f;
                                if (entities[idx].stateValue < 0.0f) entities[idx].stateValue += 360.0f;
                            }
                        }
                    }
                } 
                else { // NORMAL SELECTION & DRAGGING MODE
                    ...
                    if (isDragging && !selectedIndices.empty()) {
                        ...
                        // --- FEATURE: Height Placement Logic ---
                        if (ctrlPressed) {
                            float heightDelta = -GetMouseDelta().y * 1.0f; 
                            for (size_t i = 0; i < selectedIndices.size(); ++i) {
                                entities[selectedIndices[i]].position.y += heightDelta;
                                // Lock in the new height offset for when you release Ctrl
                                dragOffsets[i].y = entities[selectedIndices[i]].position.y;
                            }
                        } else {
                            for (size_t i = 0; i < selectedIndices.size(); ++i) {
                                ...
                                // The Y value naturally maintains whatever it was set to by the dragOffsets
                            }
                        }
                    }
            ...
            if (pendingSpawnIdx != -1) {
                ...
                if (mousePos.x > UI_WIDTH && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                    SaveState(entities); 
                    std::string dName = spawnList[pendingSpawnIdx].name;      
        ...
        if (currentMode == MODE_LEVEL) {
            ...
            // --- FEATURE: Inform the user when advanced rotation mode is enabled ---
            if (isRotationMode) {
                DrawText("ROTATION MODE ACTIVE", UI_WIDTH + 20, 20, 20, GREEN);
                DrawText("Move mouse left/right to spin. Press Left Alt to exit.", UI_WIDTH + 20, 45, 15, WHITE);
            }
            ...
            for (size_t i = 0; i < spawnList.size(); ++i) {
                ...
                if (btnY > -100 && btnY < GetScreenHeight()) { 
            ...
            // --- FIX: Segfault Guard. Prevents crashing if a box array is entirely empty! ---
            if (boxes.empty()) {
                boxes.push_back({{-20, 0, -20}, {20, 40, 20}});
            }
